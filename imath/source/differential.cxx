...
differential::differential(const ex& e_, const bool partial_, const ex& grade_, const ex& parent_,
...
differential::~differential() { MSG_INFO(3, "Destructing differential for " << e << endline); }
#endif
...
void differential::do_print_imath(const imathprint& c, unsigned level, const bool is_complete,
                                  const ex& pdiffto) const
{
    (void)level;
    int gr = get_ngrade();
    ...
    if ((partial && (difftype == "dot")) || !is_complete)
        difftype = "dfdt";
    ...
    if (difftype == "dot")
    {
        if (is_a<func>(e) && ex_to<func>(e).get_numargs() != 1)
            MSG_WARN(0, "Warning: Diff type 'dot' makes no sense with " << e << endline);
        if (!grade.info(info_flags::posint))
            throw std::logic_error(
                "Error: Diff type 'dot' is not implemented for non-positive integer diff levels");
        if (gr > 2)
            throw std::logic_error(
                "Error: Diff type 'dot' is not implemented for diff levels other than 1 or 2");
    ...
    else if (difftype == "line")
    {
        if (is_a<func>(e) && ex_to<func>(e).get_numargs() != 1)
            MSG_WARN(0,
                     "Warning: Diff type 'line' makes no sense with " << ex_to<func>(e) << endline);
    ...
    else
    {
        ...
        if (partial)
            ...
            d_sign
                = "d"; // TODO: German standard is an upright "d" but that gives too much space behind. So we stay with American italic "d" for now
...
ex differential::eval() const
{
    ...
    if (is_a<differential>(e) && (partial == ex_to<differential>(e).is_partial()))
    {
        ...
        return dynallocate<differential>(ed.e, partial, grade + ed.grade, ed.parent, ed.numerator)
...
size_t differential::nops() const { return 2; }
...
ex differential::expand(unsigned options) const
{
    ...
    const basic& newdiff = (new differential(ee, partial, gg, parent, numerator))
...
ex differential::subs(const exmap& m, unsigned options) const
{
    ...
    if (are_ex_trivially_equal(e, subsed_e) && are_ex_trivially_equal(grade, subsed_g)
        && are_ex_trivially_equal(parent, subsed_p))
        return subs_one_level(m, options);

    return differential(subsed_e, partial, subsed_g, subsed_p, numerator)
...
ex differential::derivative(const symbol& s) const
{
    ...
    return expression(differential(e, partial, grade + _ex1, parent, true))
           / expression(differential(s, partial, _ex1, e,
                                     false)); // Increase the grade of the differential
}
