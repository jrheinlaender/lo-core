                      ...
                      const expression& arhs, eqrec* eqr)
{
    ...
    for (unsigned r = 0; r < names.size(); ++r)
    {
        ...
        if (is_quantity(rh(r, 0)))
        {
            if (varr.has_value())
            {
                MSG_ERROR(0, "Warning: New value "
                                 << ex(arh(r, 0)) << " given for " << names[r] << " in "
                                 << eqr->label << ". Previous value was " << varr.val << " in "
                                 << varr.assignments.front()->label << ". Overwriting old value. "
                                 << endline);
...
void eqc::check_and_register(const expression& eq, const std::string& l)
{
    ...
    if (!is_lib(label))
    {
        ...
        {
            MSG_INFO(2,
                     "Registered equation " << label << ": " << it_eqr_new->second.eq << endline);
        }
        else
        {
            MSG_INFO(1,
...
void eqc::deleq(eqrec_it& which)
{
    ...
    if (is_lib(label))
        MSG_WARN(0, "Warning: Requesting deletion of " << label << ", which is a library equation!"
                                                       << endline);
    ...
    else
    {
        ...
        {
            // Note: Using _cit here breaks the debian build on trusty
            eqreclist_it it_assignmenteqr = std::find(v.second.assignments.begin(),
            ...
            { // The label is contained in the list
                ...
                MSG_INFO(1, "Removed " << label << " from list of assignments for " << v.first
                                       << endline);
...
symrec::symrec()
    : sym(dynallocate<extsymbol>("empty symbol"))
{
...
symrec::~symrec() { MSG_INFO(3, "Destructing symrec for " << sym << endline); }
#endif
...
expression eqc::getsym(const std::string& varname, const symprop p)
{
    ...
    { // create a new variable
        ...
        const symrec_it& v = (vars.emplace(std::piecewise_construct, std::forward_as_tuple(varname),
...
bool eqc::find_values(const symbol& var, numeric& val, expression& unit, expression& value,
                      const lst& assgn, const bool tofloat)
{
...
numeric eqc::find_numval_of(const symbol& var, const lst& assgn)
{
    ...
    if (!u.is_equal(_ex1))
        MSG_ERROR(0, "Warning: Numerical value requested, but units were found for " << var
                                                                                     << endline);
    return (v);
} //eqc::find_numval_of()
...
expression eqc::find_units_of(const symbol& var, const lst& assgn)
{
    ...
    if (v != 1)
        MSG_ERROR(0,
                  "Warning: Units requested, but numerical value was found for " << var << endline);
    return (u);
} //eqc::find_units_of()
    ...
    for (const_preorder_iterator i = e.preorder_begin(); i != e.preorder_end(); ++i)
    {
        if (is_a<symbol>(*i) && (result.find(ex_to<symbol>(*i)) == result.end()))
        {
            MSG_INFO(3, "Found contained symbol " << *i << " with value "
                                                  << get_value(ex_to<symbol>(*i)) << endline);
...
void eqc::clear()
{
    ...
    for (eqrec_it it_eqr = equations.begin(); it_eqr != equations.end();)
    {
        ...
        {
            MSG_INFO(3,
                     "Deleting equation " << it_eqr->first << ": " << it_eqr->second.eq << endline);
...
{
    MSG_INFO(3,
             "eqc::clearall, persisting symbols: " << (persist_symbols ? "yes" : "no") << endline);
    if (msg::info().checkprio(1))
        for (const auto& i : remember_split)
            msg::info() << i.second.hits << " hits for " << i.second.e << endline;
    remember_split.clear();
