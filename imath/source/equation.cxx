...
// constructors
equation::equation()
    : inherited(_ex0, _ex0, relational::equal)
    , mod(_expr0)
{
...
equation::equation(const relational& r, const expression& m)
    : inherited(r.lhs(), r.rhs(), relational::equal)
    , mod(m)
{
...
equation::~equation() { MSG_INFO(3, "Destructing equation " << lh << " = " << rh << endline); }
#endif
...
bool equation::is_assignment() const
{
    ...
    if (!is_a<extsymbol>(lh) || (o != relational::equal))
        return false;
...
void equation::do_print_imath(const imathprint& c, unsigned level) const
{
    ...
    c.s << " " << oper
        << " "; // Spaces are required for the case y <-x which will print an arrow instead of y < -x
    if (is_a<Unit>(rh))
        c.s << "1 ";
...
{
    return dynallocate<equation>(expression(lh).subs(e, options), expression(rh).subs(e, options),
                                 o, mod);
}

ex equation::subs(const exmap& m, unsigned options) const
{
    return dynallocate<equation>(expression(lh).subs(m, options), expression(rh).subs(m, options),
                                 o, mod);
}
...
{
    return dynallocate<equation>(expression(lh).csubs(m, options), expression(rh).csubs(m, options),
                                 o, mod);
}

expression equation::csubs(const ex& e, unsigned options) const
{
    return dynallocate<equation>(expression(lh).csubs(e, options), expression(rh).csubs(e, options),
                                 o, mod);
}
...
ex equation::expand(unsigned options) const
{
    ...
    return dynallocate<equation>(expression(lh).expand(options), expression(rh).expand(options), o,
                                 mod);
}
...
ex equation::eval() const
{
    ...
    if (check_modulus(mod))
    {
        ...
        return dynallocate<equation>(apply_modulus(lh, mod), apply_modulus(rh, mod), o, mod)
...
expression equation::diff(const ex& var, const expression& nth, const bool toplevel) const
{
    ...
    return dynallocate<equation>(expression(lh).diff(var, nth, toplevel),
                                 expression(rh).diff(var, nth, toplevel), o, mod);
}

ex equation::derivative(const symbol& s) const { return diff(s, _expr1); }

expression equation::pdiff(const ex& var, const expression& nth, const bool toplevel) const
{
    return dynallocate<equation>(expression(lh).pdiff(var, nth, toplevel),
                                 expression(rh).pdiff(var, nth, toplevel), o, mod);
}

expression equation::integrate(const ex& var, const extsymbol& integration_constant) const
{
    return dynallocate<equation>(expression(lh).integrate(var, integration_constant),
                                 expression(rh).integrate(var, integration_constant), o, mod);
}

expression equation::integrate(const ex& var, const ex& lowerbound, const ex& upperbound) const
{
    return dynallocate<equation>(expression(lh).integrate(var, lowerbound, upperbound),
                                 expression(rh).integrate(var, lowerbound, upperbound), o, mod);
}

expression equation::integrate(const ex& lvar, const extsymbol& l_integration_constant,
                               const ex& rvar, const extsymbol& r_integration_constant) const
{
    return dynallocate<equation>(expression(lh).integrate(lvar, l_integration_constant),
                                 expression(rh).integrate(rvar, r_integration_constant), o, mod);
}

expression equation::integrate(const ex& lvar, const ex& l_lowerbound, const ex& l_upperbound,
                               const ex& rvar, const ex& r_lowerbound, const ex& r_upperbound) const
{
    return dynallocate<equation>(expression(lh).integrate(lvar, l_lowerbound, l_upperbound),
                                 expression(rh).integrate(rvar, r_lowerbound, r_upperbound), o,
                                 mod);
}
...
expression qsolve(const exprvector& coeff, const int num)
{
    ...
    {
        if (num == 1)
            MSG_WARN(1, "Warning: This equation has only one solution" << endline);
        return (-help);
    }
    else
    {
        if (num > 2)
            MSG_WARN(1, "Warning: This equation has only two solutions" << endline);
        return ((num == 1) ? -help + pow(det, _expr1_2) : -help - pow(det, _expr1_2));
    }
}
...
expression csolve(const exprvector& coeff, const int num)
{
    ...
    if (coeff[1].is_zero() && coeff[2].is_zero())
    {
        ...
        switch (num)
        {
            ...
            case 1:
                return realsolution;
            case 2:
                return _expr1_2 * (_expr_1 + numeric(I) * pow(_expr3, _expr1_2)) * realsolution;
            case 3:
                return _expr1_2 * (_expr_1 - numeric(I) * pow(_expr3, _expr1_2)) * realsolution;
        }
    }
    ...
    expression q
        = _expr1_2
          * (_expr2 * pow(coeff[2], _expr3) / (numeric(27) * pow(coeff[3], _expr3))
             - coeff[2] * coeff[1] / (_expr3 * pow(coeff[3], _expr2)) + coeff[0] / coeff[3]);
    expression p = _expr1_3
                   * ((_expr3 * coeff[3] * coeff[1] - pow(coeff[2], _expr2))
    ...
    switch (num)
    {
        ...
        case 1:
            return (y1 - coeff[2] / (_expr3 * coeff[3]));
        case 2:
            return (y2 - coeff[2] / (_expr3 * coeff[3]));
        case 3:
            return (y3 - coeff[2] / (_expr3 * coeff[3]));
        default:
...
expression quarticsolve(const exprvector& coeff, const int num)
{
    ...
    if (coeff[1].is_zero() && coeff[3].is_zero())
    {
        ...
        else
        {
            ...
            switch (num)
            {
                case 0:
                case 1:
                    return sqrt(qsolve(newcoeff, 1));
                case 2:
                    return -sqrt(qsolve(newcoeff, 1));
                case 3:
                    return sqrt(qsolve(newcoeff, 2));
                case 4:
                    return -sqrt(qsolve(newcoeff, 2));
            }
        }
    ...
    expression problem = pow(x, _expr4) + coeff[3] / coeff[4] * pow(x, _expr3)
                         + coeff[2] / coeff[4] * pow(x, _expr2) + coeff[1] / coeff[4] * x
                         + coeff[0] / coeff[4];
    ...
    if (q.is_zero())
    {
        ...
        switch (num)
        {
            case 0:
            case 1:
                return pow(qsolve(bcoeff, 1), _expr1_2) - coeff[3] / (coeff[4] * _expr4);
            case 2:
                return pow(qsolve(bcoeff, 1), _expr1_2) * _expr_1 - coeff[3] / (coeff[4] * _expr4);
            case 3:
                return pow(qsolve(bcoeff, 2), _expr1_2) - coeff[3] / (coeff[4] * _expr4);
            case 4:
                return pow(qsolve(bcoeff, 2), _expr1_2) * _expr_1 - coeff[3] / (coeff[4] * _expr4);
            default:
    ...
    switch (num)
    {
        case 0: // This returns the first solution, also
        case 1:
            return qsolve(f1coeff, 1) - coeff[3] / (coeff[4] * _expr4);
        case 2:
            return qsolve(f1coeff, 2) - coeff[3] / (coeff[4] * _expr4);
        case 3:
            return qsolve(f2coeff, 1) - coeff[3] / (coeff[4] * _expr4);
        case 4:
            return qsolve(f2coeff, 2) - coeff[3] / (coeff[4] * _expr4);
        default:
            throw std::invalid_argument("Not more than four solutions exist");
    }
}
...
{
    if (!is_a<numeric>(n))
        throw std::invalid_argument("The solution number must be numerical!");
    ...
    {
        if (is_a<extsymbol>(s))
            continue; // Avoid unnecessary substitutions
    ...
    if (is_a<matrix>(e))
    {
        MSG_INFO(1, "Solving linear equation system " << *this << " for " << e << ", solution #"
                                                      << n << " requested " << endline);
        if (!is_a<matrix>(lh))
            throw std::invalid_argument("Left-hand side must be a matrix!");
        if (!is_a<matrix>(rh))
            throw std::invalid_argument("Right-hand side must be a matrix!");
        ...
        {
            if ((l.rows() != r.rows()) || (l.rows() != v.rows()))
                throw std::invalid_argument(
                    "Number of rows must be equal in linear equation system");
    ...
    MSG_INFO(1, "Solving " << *this << " for " << e << ", solution #" << n << " requested "
                           << endline);
    ...
    {
        MSG_WARN(2, "Warning: The solution may not be defined for some values because the "
    ...
    if (num > degree)
    {
        MSG_WARN(0, "Warning: This equation can only have "
                        << degree << " solutions. Returning the last one." << endline);
        num = degree;
    }

    exprvector coeff;
    for (unsigned i = 0; i <= degree; i++)
        coeff.emplace_back(problem.coeff(e_sym, i));
    ...
    switch (degree)
    {
        ...
        case 1:
            return equation(e_sym, _expr_1 * coeff[0] / coeff[1], o, mod).subs(e_subs_rev);
        case 2:
            return equation(e_sym, qsolve(coeff, num), o, mod).subs(e_subs_rev);
        case 3:
        ...
        {
            throw std::invalid_argument(
                "Solving for higher degrees than 4 is not generally possible");
...
const expression operator*(const expression& e, const expression& mul)
{
    ...
    {
        if (mul == 0)
            throw std::invalid_argument("Multiplication of equation with zero");
        equation result = ex_to<equation>(e);
    ...
    {
        if (e == 0)
            throw std::invalid_argument("Multiplication with zero");
...
const expression operator/(const expression& e, const expression& divisor)
{
    ...
    if (is_a<equation>(e) && is_a<equation>(divisor))
    { // Todo: We should check that both equations are equalities a == b
        ...
        if ((eqdividend.getop() == relational::equal) && (eqdivisor.getop() == relational::equal))
            return dynallocate<equation>(eqdividend.lhs() / eqdivisor.lhs(),
                                         eqdividend.rhs() / eqdivisor.rhs(), eqdividend.getop(),
                                         eqdividend.getmod());
        else
...
expression pow(const expression& e, const expression& exponent)
{
    ...
    if (is_a<equation>(e))
    {
        ...
        return dynallocate<equation>(dynallocate<power>(eq.lhs(), exponent),
                                     dynallocate<power>(eq.rhs(), exponent), eq.getop(),
                                     eq.getmod());
    }
    else
...
const expression operator-(const expression& e) { return e * _expr_1; }
}
