...
exderivative::exderivative(const exderivative& other)
    : basic(other)
    , numer(other.numer)
    , denom(other.denom)
{
...
exderivative::exderivative(const differential& n, const ex& d)
    : numer(n)
    , denom(_ex1)
{
        ...
        if (is_a<power>(f))
        {
            ...
            if (!is_a<differential>(get_basis(p)))
                throw std::invalid_argument(
                    "exderivative: Denominator may only contain differentials and their powers");

            differential df = ex_to<differential>(
                get_basis(p)); // no const& because compiler warns about dangling reference
    ...
    {
        denom *= dynallocate<power>(
            dynallocate<differential>(f.first, numer.is_partial(), _ex1, numer.argument(), false),
            f.second);
        grade += f.second;
    }
...
exderivative::~exderivative() { MSG_INFO(3, "Destructing exderivative for " << *this << endline); }
#endif
...
void exderivative::do_print_imath(const imathprint& c, unsigned level) const
{
    ...
    if ((c.poptions->find(o_difftype) != c.poptions->end())
        && ((*c.poptions)[o_difftype].value.str != NULL))
        difftype = *(*c.poptions)[o_difftype].value.str;

    if (difftype == "dot")
    {
        if (numer.is_partial())
            throw std::logic_error("Diff type 'dot' makes no sense for partial differentials");
        if (is_a<mul>(denom))
            throw std::logic_error(
                "Diff type 'dot' is not possible for differentials of more than one variable");
    ...
    else if (difftype == "line")
    {
        if (is_a<mul>(denom))
            throw std::logic_error(
                "Diff type 'line' is not possible for differentials of more than one variable");
        ...
        if (numer.is_partial())
        {
            ...
            if (is_a<extsymbol>(arg) || (is_a<func>(arg) && ex_to<func>(arg).is_pure()))
                ...
                throw std::logic_error("Warning: Partial differentials with diff type 'line' must "
                                       "differentiate to a symbol or function");
        }
        else
        {
...
size_t exderivative::nops() const { return 2; }
...
ex exderivative::derivative(const symbol& s) const
{
    ...
    if (is_a<add>(argument))
    {
        ...
        for (const auto& a : argument)
            result += exderivative(
                dynallocate<differential>(a, numer.is_partial(), numer.get_grade()), denom);
    ...
    else if (is_a<mul>(argument))
    {
        ...
        for (const auto& m : argument)
        {
            ...
            for (const auto& m2 : argument)
            {
                if (m == m2)
                    result *= exderivative(
                        dynallocate<differential>(m2, numer.is_partial(), numer.get_grade()),
                        denom);
                else
                    result *= m;
            }
    ...
    else if (is_a<power>(argument))
    {
        ...
        ex result = get_exp(p) * dynallocate<power>(get_basis(p), get_exp(p) - _ex1)
                    * exderivative(dynallocate<differential>(get_basis(p), numer.is_partial(),
                                                             numer.get_grade()),
                                   denom);
        return result.diff(s);
    }
...
ex exderivative::eval_diff() const
{
    ...
    ex the_differential
        = numer
          / denom; // The not differentiated exderivative, will be returned if differentiating is not possible
    if (numer.get_ngrade() == -1)
        return the_differential; // non-numeric grades can't be differentiated
    ...
    for (const auto& f : (is_a<mul>(denom) ? denom : lst{ denom }))
    {
        ...
        {
            const power& p = ex_to<power>(f);
            if (!get_exp(p).info(info_flags::posint))
    ...
    for (const auto& v : vars)
    {
        ...
        MSG_INFO(2, "Differentiating " << result << " to " << diffarg << " grade " << grade
                                       << endline);
        ...
        {
            result
                = dynallocate<exderivative>(dynallocate<differential>(result, partial, grade), v);
        }

        MSG_INFO(2, "Result after differentiating " << grade << " times to " << diffarg << ": "
                                                    << result << endline);
    }
