...
expression expression::csubs(const unitvec& v, unsigned options) const
{
    ...
    expression expression::subsv(const expression& e, const bool consecutive, unsigned options)
        const
    {
        // The expression must have form {var1 = vector1; var2 = vector2; ... }
        unsigned rows = 0;
        ...
        for (const auto& eq : e)
        {
            if (!eq.info(info_flags::relation_equal))
                throw std::invalid_argument(
                    "The expression to substitute must be a relational or a list of relationals.");
            if (!is_a<matrix>(eq.op(1)))
                throw std::runtime_error("The expression to substitute must have a column vector "
            ...
            {
                if (m.rows() != rows)
                    throw std::runtime_error("All expressions to substitute must have the same "
                                             "number of vector elements");
                unsigned i = 0;
                for (std::vector<exmap>::iterator s = substitutions.begin();
                     s != substitutions.end(); ++s)
                {
                    s->insert(std::make_pair(eq.op(0), m[i]));
                    ++i;
                }
        ...
        for (const auto& i : m)
        {
            ...
            struct expand_sum : public map_function
            {
                ex operator()(const ex& e)
                {
                    ...
                    if (is_a<func>(e) && (ex_to<func>(e).get_name() == "sum"))
                    {
                        ...
                        extsymbol var
                            = ex_to<extsymbol>(ex_to<equation>(fargs[0])
            ...
            expression expression::csubs(const unitvec& v, unsigned options) const
            {
                ...
                for (const auto& i : v)
                {
                    ...
                    {
                        throw std::invalid_argument("The expression to substitute must be a "
                ...
                expression expression::csubs(const ex& e, unsigned options) const
                {
                    ...
                    else if (e.info(info_flags::list))
                    {
                        ...
                        for (const auto& i : ex_to<lst>(e))
                        {
                            ...
                            result = result.subs(m, options);
                            MSG_INFO(2,
                                     "Substituted " << m << ", result: " << ex(result) << endline);
                        }
                        return result;
                    }
                    else
                ...
                struct expand_sum : public map_function
                {
                    ...
                    {
                        MSG_INFO(2, "Collecting expression " << ex(*this) << " to symbol " << e
                                                             << endline);
                        ex sym = e;
                        if (e.is_zero())
                        {
                    ...
                    expression expression::diff(const expression& var, const expression& nth,
                                                bool toplevel) const
                    {
                        MSG_INFO(1, "Calculating " << nth << "-th derivative of " << ex(*this)
                                                   << " to " << var << endline);
                        ...
                        if (!nth.info(info_flags::nonnegint))
                        {
                            ...
                            if (toplevel)
                                return expression(
                                           dynallocate<differential>(*this, false, nth, _ex0, true))
                                       / expression(dynallocate<differential>(var, false, nth,
                                                                              *this, false));
                            else
                        ...
                        if (is_a<extsymbol>(var))
                        {
                            ...
                            {
                                throw std::logic_error("Can only differentiate with respect to a "
                        ...
                        ex replace_pure_funcs_by_symbols::operator()(const ex& e)
                        {
                                ...
                                && (!e.is_equal(diffvar)))
                            {
                                ...
                                MSG_INFO(2,
                                         "Replacing pure func " << e << " with symbol" << endline);
                        ...
                        expression expression::pdiff(const expression& var, const expression& nth,
                                                     bool toplevel) const
                        {
                            MSG_INFO(1, "Calculating " << nth << "-th partial derivative of "
                    ...
                    void expression::print(const print_context& c, unsigned level) const
                    {
                        ...
                        expression expression::integrate(const ex& var,
                                                         const symbol& integration_constant) const
                        {
                            extintegral result(var, *this, integration_constant);
                            return result.eval_integ();
                        }
                        ...
                        {
                            MSG_INFO(3,
                                     "Assigning expression from expression " << ex(e) << endline);
                            ex::operator=(e);
                            empty = e.empty;
                            return *this;
                        }
