...
extsymbol::extsymbol()
    : symbol()
    , _domain(domain::complex)
    , _return_type(return_types::commutative)
{
    MSG_INFO(3, "Constructing empty extsymbol" << endline);
}

extsymbol::extsymbol(const std::string& initname)
    : symbol(initname)
    , _domain(domain::complex)
    , _return_type(return_types::commutative)
{
...
extsymbol::extsymbol(const std::string& initname, const std::string& texname)
    : symbol(initname, texname)
    , _domain(domain::complex)
    , _return_type(return_types::commutative)
{
    MSG_INFO(3,
             "Constructing extsymbol with " << initname << " and textname " << texname << endline);
}
