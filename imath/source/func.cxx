...
GINAC_IMPLEMENT_REGISTERED_CLASS_OPT(
    func, exprseq,
    print_func<print_context>(&func::do_print).print_func<imathprint>(&func::do_print_imath));
...
func::~func() { MSG_INFO(3, "Destructing func " << name << endline); }
#endif

func::func(const std::string& n, const exprseq& args, const unsigned s, const bool hc,
           const exvector& v, const expression& d, const unsigned h, const std::string& pn)
    : exprseq(args)
    , name(n)
    , serial(s)
    , hard(hc)
    ...
    , printname(pn)
{
    MSG_INFO(2, "Constructing " << (hard ? "hard-coded " : "") << "func from exprseq with name "
                                << n << " and arguments " << seq << endline);
}
func::func(const std::string& n, exprseq&& args, const unsigned s, const bool hc, const exvector& v,
           const expression& d, const unsigned h, const std::string& pn)
    : exprseq(std::move(args))
    , name(n)
    , serial(s)
    , hard(hc)
    , vars(v)
    ...
    , printname(pn)
{
    MSG_INFO(2, "Moving " << (hard ? "hard-coded " : "") << "func from exprseq with name " << n
                          << " and arguments " << seq << endline);
}
...
void func::printseq(const print_context& c, const std::string& openbracket,
                    const std::string& delim, const std::string& closebracket) const
{
...
void func::do_print_imath(const imathprint& c, unsigned level) const { print_imath(c, 1, level); }
...
#pragma warning(pop)
    ...
    if (hard)
    {
        if ((hints & FUNCHINT_PRINT) && !seq.empty())
        {
            ...
            function(serial, seq)
                .print(c, level + 1); // Fall through to the hard-coded print function
            return;
        }
        else
        {
            ...
            if (name == "fact")
            {
                ...
                if (is_a<numeric>(op(0)) || is_a<extsymbol>(op(0)) || is_a<power>(op(0))
                    || is_a<Unit>(op(0))
                    || (is_a<func>(op(0)) && !ex_to<func>(op(0)).is_lib())) // do not print brackets
                    seq[0].print(c);
                else
                    printseq(c, "(", delim, ")");
                return;
    ...
    if (p != 1)
    { // Print the exponent of a trigonometric function directly after the function name
        c.s << printname << "^";
        p.print(c, level + 1);
        if (seq.empty())
            return;
    ...
    {
        if (!hard && is_lib()
            && name[0] != '%') // func does not work in combination with e.g. %alpha
    ...
    if (is_nobracket())
    { // do not print brackets
        printseq(c, "{", delim, "}");
    }
    else if (is_trig() && (seq.size() == 1)
             && (is_a<numeric>(seq[0]) || is_a<extsymbol>(seq[0]) || is_a<power>(seq[0])
                 || is_a<Unit>(seq[0]) || (is_a<func>(seq[0]) && !ex_to<func>(seq[0]).is_lib())))
    {
        c.s << "{";
...
void func::print_diff_line(const ex& g, const int gr, const print_context& c) const
{
    ...
    std::string lines = "";
    std::string diffchar
        = "%d1"; // Special character from Math catalog 'iMath', see iMathFormatting.xcu
    if (gr > 0)
        for (int i = 0; i < gr; i++)
            lines += diffchar;
    lines = "^{" + lines + "}";
...
#pragma warning(pop)
    ...
    if (!hard && is_lib())
        fname
            = "func "
              + fname; // so that sec() etc. are not printed in italics like user-defined functions
...
expression func::expand_definition() const
{
    ...
    MSG_INFO(2, "Expanding user function " << name << "(" << subst << ") = " << definition
                                           << endline << "Expansion result: " << result << endline);
    return (result);
}
...
ex func::expand(unsigned options) const
{
    ...
    { // Only expand arguments when asked to do so
        if (name == "mindex")
        { // Special treatment, because we don't want the indices to became floats...
    ...
    if (!definition.is_empty() && !(options & no_expand_function_definition))
    { // is there a function definition that can be expanded?
        try
        {
            return (
                result.expand_definition()); //TODO: should we do another expand() on this result,
