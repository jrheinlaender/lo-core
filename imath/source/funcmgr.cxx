...
const std::map<std::string, unsigned> funchint_map
    = { { "none", FUNCHINT_NONE },           { "lib", FUNCHINT_LIB },
        { "trig", FUNCHINT_TRIG },           { "expand", FUNCHINT_EXPAND },
        { "nobracket", FUNCHINT_NOBRACKET }, { "defdiff", FUNCHINT_DEFDIFF },
        { "print", FUNCHINT_PRINT } };

// Define static members------------------------------------------------------------------
std::map<const std::string, Functionmanager::funcrec> Functionmanager::hard_functions
    = std::map<const std::string, funcrec>();
std::map<const std::string, std::string> Functionmanager::hard_names
...
expression Functionmanager::replace_function_by_func(const ex& e)
{
    struct replace_function_by_func : public map_function
    {
        ...
        {
            MSG_INFO(3,
                     "Replacing all hard-coded GiNaC functions by EQC funcs in " << e << endline);
            ...
            if (is_a<fderivative>(e))
            {
                ...
                if (parset.empty())
                    throw std::runtime_error(
                        "Internal error: fderivative with no parameters in paramset");
                if (parset.size() > 1)
                    throw std::runtime_error(
                        "Internal error: Multiple parameters to fderivative not yet supported");

                // Name of the function being differentiated
                std::string fname = Functionmanager::get_hard_name(f.get_name());
                if (fname.empty())
                    throw std::runtime_error("Internal error: Unknown hard-coded function "
                                             + f.get_name() + " in expression");
                unsigned paridx = *parset.begin();
                if (msg::info().checkprio(2))
                {
                    msg::info() << "fderivative: index = " << paridx << "; function name: " << fname
                                << "; arguments: ";
                    for (const auto& i : f)
                        msg::info() << i << ", ";
                    msg::info() << endline;
                ...
                ex the_function
                    = dynallocate<func>(fname, exprseq(f.begin(), f.end()), fr.serial, fr.hard,
                                        fr.vars, fr.definition, fr.hints, fr.printname);
                if (paridx > ex_to<func>(the_function).get_numargs())
                    throw std::runtime_error("Error: Differentiation to non-existent variable");
                return dynallocate<exderivative>(
                    dynallocate<differential>(the_function, false, _ex1),
                    dynallocate<differential>(f.op(paridx), false, _ex1, the_function, false));
            }
            else if (is_a<function>(e))
            {
                ex e_mapped = e.map(*this);
                if (!is_a<function>(e_mapped))
                    return e_mapped;

                std::string fname = Functionmanager::get_hard_name(ex_to<function>(e).get_name());
                if (fname.empty())
                    throw std::runtime_error("Internal error: Unknown hard-coded function "
                                             + ex_to<function>(e).get_name() + " in expression");

                const auto& fr = Functionmanager::hard_functions.find(fname)->second;
                const function& f = ex_to<function>(e_mapped);
                return dynallocate<func>(fname, exprseq(f.begin(), f.end()), fr.serial, fr.hard,
                                         fr.vars, fr.definition, fr.hints, fr.printname)
        ...
        const auto& fr = Functionmanager::hard_functions.find(fname) -> second;
        ex the_function = dynallocate<func>(fname, exprseq(f.begin(), f.end()), fr.serial, fr.hard,
                                            fr.vars, fr.definition, fr.hints, fr.printname);
        if (paridx > ex_to<func>(the_function).get_numargs())
            throw std::runtime_error("Error: Differentiation to non-existent variable");
        return dynallocate<exderivative>(
...
Functionmanager::Functionmanager()
{
        ...
        hard_functions = {
            ...
            { "arccos",
              { function::find_function("acos", 1),
                ...
                "arccos" } },
            { "arcosh",
              { function::find_function("acosh", 1),
                { x },
                true,
                empty,
                FUNCHINT_LIB | FUNCHINT_TRIG,
            ...
            { "arcsin",
              { function::find_function("asin", 1),
                ...
                "arcsin" } },
            { "arsinh",
              { function::find_function("asinh", 1),
                { x },
                true,
                empty,
                FUNCHINT_LIB | FUNCHINT_TRIG,
            ...
            { "arctan",
              { function::find_function("atan", 1),
                ...
                "arctan" } },
            { "artanh",
              { function::find_function("atanh", 1),
                { x },
                true,
                empty,
                FUNCHINT_LIB | FUNCHINT_TRIG,
            ...
            { "tanh",
              { function::find_function("tanh", 1),
                ...
                "tanh" } },
            { "re",
              { function::find_function("real_part", 1), { x }, true, empty, FUNCHINT_LIB, "Re" } },
            { "Re",
              { function::find_function("real_part", 1), { x }, true, empty, FUNCHINT_LIB, "Re" } },
            { "im",
              { function::find_function("imag_part", 1), { x }, true, empty, FUNCHINT_LIB, "Im" } },
            { "Im",
              { function::find_function("imag_part", 1), { x }, true, empty, FUNCHINT_LIB, "Im" } },
            { "conjugate",
              { function::find_function("conjugate", 1),
                { x },
                true,
                empty,
                FUNCHINT_LIB,
            ...
            { "vecprod",
              { vecprod_SERIAL::serial,
                ...
                "vecprod" } },
            { "vmax",
              { vmax_SERIAL::serial, { v1 }, true, empty, FUNCHINT_LIB | FUNCHINT_PRINT, "vmax" } },
            { "vmin",
              { vmin_SERIAL::serial, { v2 }, true, empty, FUNCHINT_LIB | FUNCHINT_PRINT, "vmin" } },
            // Hard-coded matrix functions from GiNaC matrix.h
            { "charpoly",
              { charpoly_SERIAL::serial,
                { m1, v1 },
                true,
                empty,
                FUNCHINT_LIB | FUNCHINT_PRINT,
            ...
            { "ones",
              { onesmatrix_SERIAL::serial,
                ...
                "ones" } },
            { "rank",
              { rank_SERIAL::serial, { m1 }, true, empty, FUNCHINT_LIB | FUNCHINT_PRINT, "rank" } },
            { "reducematrix",
              { reducematrix_SERIAL::serial,
                { m1, e1, e2 },
                true,
                empty,
                FUNCHINT_LIB | FUNCHINT_PRINT,
            ...
            { "submatrix",
              { submatrix_SERIAL::serial,
                ...
                "submatrix" } },
            { "tr",
              { trace_SERIAL::serial, { m1 }, true, empty, FUNCHINT_LIB | FUNCHINT_PRINT, "tr" } }
    ...
    Functionmanager::Functionmanager()
    {
        ...
        if (!initialized)
        {
            ...
            hard_functions = {
                ...
                { "fact",
                  { function::find_function("factorial", 1),
                    ...
                    "fact" } },
                { "ln",
                  { function::find_function("log", 1), { x }, true, empty, FUNCHINT_LIB, "ln" } },
                { "sin",
                  { function::find_function("sin", 1),
                    { x },
                    true,
                ...
                { "tanh",
                  { function::find_function("tanh", 1),
                    ...
                    "tanh" } },
                { "re",
                  { function::find_function("real_part", 1),
                    { x },
                    true,
                    empty,
                    FUNCHINT_LIB,
                ...
                { "Re",
                  { function::find_function("real_part", 1),
                    ...
                    "Re" } },
                { "im",
                  { function::find_function("imag_part", 1),
                    { x },
                    true,
                    empty,
                    FUNCHINT_LIB,
            ...
            {
                MSG_INFO(3, "Hard-coded function "
                                << fr.first << "(" << function(fr.second.serial).get_name()
                                << ") with serial " << fr.second.serial << endline);
    ...
    const Functionmanager::funcrec& Functionmanager::findAttributes(const std::string& fname,
                                                                    const unsigned nargs) const
    {
        ...
        MSG_INFO(2, "Found registered " << (it_frec->second.hard ? "hard-coded " : "")
                                        << "function with serial " << it_frec->second.serial
                                        << endline);
        MSG_INFO(3, "Checking variables" << endline);
        if (nargs > 0 && (nargs != it_frec->second.vars.size()))
            throw std::invalid_argument(
                "Number of arguments does not match for " + fname + ". Expected "
                + std::to_string(it_frec->second.vars.size()) + " arguments, found "
    ...
    expression Functionmanager::create(const std::string& n, const exprseq& args) const
    {
        ...
        return dynallocate<func>(n, args, fr.serial, fr.hard, fr.vars, fr.definition, fr.hints,
                                 fr.printname)
            .eval(); // The eval() is required for FUNCHINT_EXPAND functions
    }
    ...
    expression Functionmanager::create(const std::string& n, exprseq&& args) const
    {
        ...
        const funcrec& fr = findAttributes(n, args.nops());
        return dynallocate<func>(n, std::move(args), fr.serial, fr.hard, fr.vars, fr.definition,
                                 fr.hints, fr.printname)
            .eval(); // The eval() is required for FUNCHINT_EXPAND functions
    }
                                            ...
                                            const bool eval)
    {
        ...
        if (args.nops() != fr.vars.size())
            throw std::invalid_argument("Number of arguments does not match for " + n
                                        + ". Expected " + std::to_string(fr.vars.size())
                                        + " arguments, found " + std::to_string(args.nops())
                                        + " arguments");

        expression result(dynallocate<func>(n, args, fr.serial, true, fr.vars, fr.definition,
                                            fr.hints, fr.printname)
                              .hold());
        if (eval)
            return result.eval();
    ...
    expression Functionmanager::create_hard(const std::string& n, exprseq&& args, const bool eval)
    {
        ...
        const funcrec& fr = it_frec->second;
        expression result(dynallocate<func>(n, std::move(args), fr.serial, true, fr.vars,
                                            fr.definition, fr.hints, fr.printname)
                              .hold());
        if (eval)
            return result.eval();
    ...
    expression Functionmanager::find_integral(const std::string& name, const exprseq& seq)
    {
        if (name == "cos")
            ...
            return _ex_1
                   * create_hard(
                         "ln",
                         exprseq({ create_hard("abs", exprseq({ create_hard("cos", seq) })) }));
        else if (name == "cot")
            return create_hard(
                "ln", exprseq({ create_hard("abs", exprseq({ create_hard("sin", seq) })) }));
        else if (name == "sec")
            return create_hard(
                "ln", exprseq({ create_hard("abs", exprseq({ (1 + create_hard("sin", seq))
                                                             / create_hard("cos", seq) })) }));
        else if (name == "csc")
            return create_hard(
                "ln",
                exprseq({ create_hard("abs", exprseq({ create_hard("sin", seq)
                                                       / (1 + create_hard("cos", seq)) })) }));
        else if (name == "arcsin")
            ...
            return seq[0] * create_hard("arctan", seq)
                   - _ex1_2 * create_hard("ln", exprseq({ _ex1 + pow(seq[0], _ex2) }));
        // arccot, arcsec, arccsc: See func.cxx
            ...
            return create_hard(
                "ln", exprseq({ create_hard("abs", exprseq({ create_hard("sinh", seq) })) }));
        else if (name == "sech")
            return create_hard("arctan", exprseq({ create_hard("sinh", seq) }));
        else if (name == "csch")
            return create_hard(
                "ln", exprseq({ create_hard(
                          "abs", exprseq({ create_hard("tanh", exprseq({ seq[0] / _ex2 })) })) }));
        else if (name == "arsinh")
            ...
            return (seq[0] * create_hard("artanh", seq)
                    + _ex1_2 * create_hard("ln", exprseq({ _ex1 - pow(seq[0], _ex2) })));
        // arcoth, arsech, arcsch: See func.cxx
        else if (name == "exp")
            return create_hard("exp", seq);
        else if (name == "log")
            return _ex1 / (create_hard("ln", exprseq({ numeric(10) })))
                   * (seq[0] * create_hard("ln", seq) - seq[0]);
        else if (name == "ln")
    ...
    unsigned Functionmanager::hint(const std::string& s)
    {
        ...
        if (s == "no_bracket")
        {
            MSG_WARN(0, "Warning: Replace deprecated function hint 'no_bracket' with 'nobracket'"
                            << endline);
            str = "nobracket";
        }
    ...
    void Functionmanager::registr(const std::string& n, const exvector& args, const unsigned h,
                                  const std::string& printname)
    {
        static const unsigned max_unsigned
            = unsigned(1 << (sizeof(unsigned) * 8 - 1)); // *** TODO: 1 << ...*8 gives warning

        MSG_INFO(3, "Registering function " << n << " '" << printname << "' with arguments " << args
                                            << endline);
        if (hard_functions.find(n) != hard_functions.end()
            || user_functions.find(n) != user_functions.end())
        ...
        auto f = user_functions
                     .emplace(n, funcrec(serial, std::move(args), false, expression(), h,
                                         (printname == "" ? n : printname)))
                     .first->second;

        MSG_INFO(2, "Registered function "
                        << f.printname << "(" << f.vars << ") with serial " << f.serial
                        << ((h & FUNCHINT_LIB) ? ", which is a library function." : ".")
                        << endline);
    }
    ...
    void Functionmanager::define(const std::string& n, const expression& def)
    {
        ...
        if (it_func == user_functions.end())
            throw(std::invalid_argument("Function " + n
                                        + " does not exist! Please register it first. Or it is a "
                                          "hard-coded function and cannot be defined"));

        it_func->second.definition = def;
        MSG_INFO(2, "Defined function " << n << "(" << it_func->second.vars << ")  = " << def
                                        << endline);
    }
    ...
    bool Functionmanager::is_a_func(const std::string& fname) const
    {
        ...
        return (hard_functions.find(fname) != hard_functions.end()
                || user_functions.find(fname) != user_functions.end());
    }
    ...
    {
        if (hard_functions.find(fname) != hard_functions.end())
            return true;
