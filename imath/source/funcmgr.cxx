...
Functionmanager::Functionmanager()
{
    ...
    if (!initialized)
    {
        ...
        {
            MSG_INFO(3, "Hard-coded function " << fr.first << "("
                                               << function(fr.second.serial).get_name()
                                                                ...
                                                                const unsigned nargs) const
{
    ...
    if (nargs > 0 && (nargs != it_frec->second.vars.size()))
        throw std::invalid_argument("Number of arguments does not match for " + fname
                                    + ". Expected " + std::to_string(it_frec->second.vars.size())
                                    + " arguments, found " + std::to_string(nargs) + " arguments");
...
expression Functionmanager::create_hard(const std::string& n, exprseq&& args)
{
    ...
    const funcrec& fr = it_frec->second;
    return expression(dynallocate<func>(n, std::move(args), fr.serial, true, fr.vars, fr.definition,
                                        fr.hints, fr.printname)
                          .hold());
}
