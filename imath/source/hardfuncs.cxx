    ...
    if (is_a<matrix>(e1) && is_a<matrix>(e2))
    {
        ...
        if (!(((v1.cols() == 1) && (v2.cols() == 1) && (v1.rows() == v2.rows()) && (v1.rows() == 3))
              || ((v1.rows() == 1) && (v2.rows() == 1) && (v1.cols() == v2.cols())
                  && (v1.cols() == 3))))
            throw std::invalid_argument("Error: The vector product can only be calculated with two "
...
REGISTER_FUNCTION(vecprod, eval_func(vecprod_eval).print_func<imathprint>(vecprod_print_imath));
...
REGISTER_FUNCTION(scalprod, eval_func(scalprod_eval).print_func<imathprint>(scalprod_print_imath));
...
static ex ifelse_evalf(const ex& condition, const ex& e1, const ex& e2)
{
    ...
    int result = eval_condition(e_condition);
    if (result == -1)
        return Functionmanager::create_hard("ifelse",
                                            exprseq{ e_condition, e1.evalf(), e2.evalf() });
...
REGISTER_FUNCTION(onesmatrix,
                  eval_func(onesmatrix_eval).print_func<imathprint>(onesmatrix_print_imath));
...
REGISTER_FUNCTION(charpoly, eval_func(charpoly_eval).print_func<imathprint>(charpoly_print_imath));
...
static ex solvematrix_eval(const ex& e, const ex& vars, const ex& rhs)
{
    ...
    MSG_INFO(3,
             "solvematrix: " << e << " for " << vars << " with right-hand side " << rhs << endline);
