...
static ex round_eval(const ex& e, const ex& n)
{
    ...
    if (digits.is_zero())
        return dynallocate<numeric>(
            truncate1(cln::the<cln::cl_R>(num.to_cl_N())
                      + csgn(num) * cln::cl_float(0.5, cln::default_float_format)));

    // Note: Converting digits to a long limits the number of decimal places that can be rounded to 2^64
    cln::cl_F m = cln::the<cln::cl_F>(cln::expt(cln::cl_float(10.0, cln::default_float_format),
                                                cln::the<cln::cl_I>(digits.to_long())));
    return dynallocate<numeric>(
        truncate1(cln::the<cln::cl_R>(num.to_cl_N()) * m
                  + csgn(num) * cln::cl_float(0.5, cln::default_float_format))
        / m);
}
...
static ex floor_eval(const ex& e, const ex& n)
{
    ...
    if (!is_a<numeric>(n))
    {
        ex ndig
            = expression(n)
                  .evalf(); // Maybe n becomes a numeric now? Note: Doing evalf() right away converts integers to floats
    ...
    if (digits.is_zero())
        return dynallocate<numeric>(floor1(cln::the<cln::cl_R>(num.to_cl_N())));
...
static ex ceil_eval(const ex& e, const ex& n)
{
    ...
    if (digits.is_zero())
        return dynallocate<numeric>(ceiling1(cln::the<cln::cl_R>(num.to_cl_N())));
...
REGISTER_FUNCTION(sum, eval_func(sum_eval).print_func<imathprint>(sum_print_imath));

namespace
{
std::pair<GiNaC::expression, GiNaC::expression> split_scalar_and_matrix(const GiNaC::expression& e,
                                                                        const bool vector)
{
...
static ex mindex_eval(const ex& e, const ex& r, const ex& c)
{
    ...
    if (is_a<matrix>(matr))
    {
        ...
        if (is_a<numeric>(r))
        {
            ...
            { // try harder, since non-integer indixes make no sense
                row = numeric_to_int(rnum);
            }
            row--; // Adjust index to count from 0
            if (row >= (int)m.rows() || (row < 0 && row != -1000))
            {
                MSG_WARN(0, "mindex: Warning: Row index " << row + 1 << " out of bounds for " << m
                                                          << endline);
                return unchanged;
            }
        }
        ...
        if (is_a<numeric>(c))
        {
            ...
            {
                MSG_WARN(0, "mindex: Warning: Column index " << col + 1 << " out of bounds for "
                                                             << m << endline);
                return unchanged;
            }
        }
        ...
        if (row == -1)
        {
            ...
            else
            {
                ...
                {
                    return scalar
                           * m(0, col); // Special treatment for vector: Return one element only
                }
                else
                {
        ...
        else
        {
            if (col == -1)
            {
                ...
                {
                    return scalar
                           * m(row, 0); // Special treatment for vector: Return one element only
                }
                else
                {
...
static ex hadamard_eval(const ex& e1, const ex& e2, const ex& op)
{
    ...
    MSG_INFO(1, "hadamard eval: " << e1
                                  << (h_op == h_product ? "*" : (h_op == h_division ? "/" : "^"))
                                  << e2 << endline);
    ...
    if (is_a<matrix>(e1_e) && is_a<matrix>(e2_e))
    {
            ...
            for (unsigned r = 0; r < rows; ++r)
            {
                for (unsigned c = 0; c < cols; ++c)
                {
                    ...
                    if (is_a<numeric>(c))
                    {
                        ...
                        if (col >= (int)m.cols() || (col < 0 && col != -1000))
                        {
                            MSG_WARN(0, "mindex: Warning: Column index "
                                            << col + 1 << " out of bounds for " << m << endline);
                            return unchanged;
                        }
                    }
                    else if (is_a<wildcard>(c))
                    ...
                    if (row == -1)
                    {
                        ...
                        else
                        {
                            if (m.rows() == 1)
                            {
                                return m(
                                    0,
                                    col); // Special treatment for vector: Return one element only
                            }
                            else
                            {
                                matrix result(m.rows(), 1);
                            ...
                            MSG_INFO(2,
                                     "mindex row: " << row << ", rows(): " << m.rows() << endline);
                            ...
                            MSG_INFO(2,
                                     "mindex col: " << col << ", cols(): " << m.cols() << endline);
                            ...
                            else
                            {
                                if (col == -1)
                                {
                                    if (m.cols() == 1)
                                    {
                                        return m(
                                            row,
                                            0); // Special treatment for vector: Return one element only
                                    }
                                    else
                                    {
                                        matrix result(1, m.cols());
                    ...
                    static void mindex_print_imath(const ex& e, const ex& row, const ex& col,
                    ...
                    REGISTER_FUNCTION(
                        mindex, eval_func(mindex_eval).print_func<imathprint>(mindex_print_imath));
                    ...
                    static ex hadamard_eval(const ex& e1, const ex& e2, const ex& op)
                    {
                        ...
                        MSG_INFO(1,
                                 "hadamard eval: "
                                     ...
                                     << (h_op == h_product ? "*" : (h_op == h_division ? "/" : "^"))
                                     << e2 << endline);
                        ex unchanged = Functionmanager::create_hard("hadamard",
                                                                    exprseq{ e1, e2, op }, false);
                    ...
                    static void hadamard_print_imath(const ex& e1, const ex& e2, const ex& op,
                                                     const print_context& c)
                    {
                    ...
                    REGISTER_FUNCTION(
                        hadamard,
                        eval_func(hadamard_eval).print_func<imathprint>(hadamard_print_imath));
                    ...
                    REGISTER_FUNCTION(
                        transpose,
                        eval_func(transpose_eval).print_func<imathprint>(transpose_print_imath));
                    ...
                    static ex vecprod_eval(const ex& e1, const ex& e2)
                    {
                        ...
                        if (is_a<matrix>(e1) && is_a<matrix>(e2))
                        {
                            ...
                            if (!(((v1.cols() == 1) && (v2.cols() == 1) && (v1.rows() == v2.rows())
                                   && (v1.rows() == 3))
                                  || ((v1.rows() == 1) && (v2.rows() == 1)
                                      && (v1.cols() == v2.cols()) && (v1.cols() == 3))))
                                throw std::invalid_argument(
                                    "Error: The vector product can only be calculated "
                                    "with two row or column vectors of three "
                                    "components each");
                    ...
                    static ex scalprod_eval(const ex& e1, const ex& e2)
                    {
                        ...
                        ex unchanged
                            = Functionmanager::create_hard("scalprod", exprseq{ e1, e2 }, false);
                        ...
                        if (is_a<matrix>(e1) && is_a<matrix>(e2))
                        {
                            ...
                            if (!(((v1.cols() == 1) && (v2.cols() == 1) && (v1.rows() == v2.rows())
                                  ...
                                  || ((v1.rows() == 1) && (v2.rows() == 1)
                                      && (v1.cols() == v2.cols()) && (v1.cols() == 3))))
                                throw std::invalid_argument(
                                    "Error: The vector product can only be calculated with two "
                                    "row or column vectors of three components each");
                    ...
                    static void vecprod_print_imath(const ex& e1, const ex& e2,
                                                    const print_context& c)
                    {
                        e1.print(c);
                        c.s << " times ";
                    ...
                    REGISTER_FUNCTION(
                        vecprod,
                        eval_func(vecprod_eval).print_func<imathprint>(vecprod_print_imath));
                    ...
                    static void scalprod_print_imath(const ex& e1, const ex& e2,
                                                     const print_context& c)
                    {
                        e1.print(c);
                        c.s << " ";
                    ...
                    REGISTER_FUNCTION(
                        scalprod,
                        eval_func(scalprod_eval).print_func<imathprint>(scalprod_print_imath));
                    ...
                    static ex ifelse_eval(const ex& condition, const ex& e1, const ex& e2)
                    {
                        ...
                        MSG_INFO(3, "eval ifelse: " << condition << " ? " << e1 << " : " << e2
                                                    << endline);
                        ...
                        if (result == -1)
                            return Functionmanager::create_hard(
                                "ifelse", exprseq{ e_condition, e1.eval(), e2.eval() }, false);
                    ...
                    static ex ifelse_evalf(const ex& condition, const ex& e1, const ex& e2)
                    {
                        ...
                        MSG_INFO(3, "evalf ifelse: " << condition << " ? " << e1 << " : " << e2
                                                     << endline);
                        ...
                        if (result == -1)
                            return Functionmanager::create_hard(
                                "ifelse", exprseq{ e_condition, e1.evalf(), e2.evalf() }, false);
                    ...
                    REGISTER_FUNCTION(
                        vmax, eval_func(vmax_eval).print_func<imathprint>(vmax_print_imath));
                    ...
                    REGISTER_FUNCTION(
                        vmin, eval_func(vmin_eval).print_func<imathprint>(vmin_print_imath));
                    ...
                    static ex concat_eval(const ex& v1, const ex& v2)
                    {
                        ...
                        ex unchanged
                            = Functionmanager::create_hard("concat", exprseq{ v1, v2 }, false);
                    ...
                    REGISTER_FUNCTION(
                        irem, eval_func(irem_eval).print_func<imathprint>(irem_print_imath));
                    ...
                    REGISTER_FUNCTION(
                        iquo, eval_func(iquo_eval).print_func<imathprint>(iquo_print_imath));
                    ...
                    REGISTER_FUNCTION(
                        diagmatrix,
                        eval_func(diagmatrix_eval).print_func<imathprint>(diagmatrix_print_imath));
                    ...
                    static void identmatrix_print_imath(const ex& r, const ex& col,
                                                        const print_context& c)
                    {
                        c.s << "func ident( ";
                        r.print(c);
                    ...
                    REGISTER_FUNCTION(identmatrix,
                                      eval_func(identmatrix_eval)
                    ...
                    static ex onesmatrix_eval(const ex& r, const ex& c)
                    {
                        ...
                        REGISTER_FUNCTION(diagmatrix,
                                          eval_func(diagmatrix_eval)
                        ...
                        static ex identmatrix_eval(const ex& r, const ex& c)
                        {
                            ...
                            return Functionmanager::create_hard("submatrix",
                                                                exprseq{ e, r, nr, c, nc }, false);
                        }
                        ...
                        REGISTER_FUNCTION(determinant,
                                          eval_func(determinant_eval)
                        ...
                        REGISTER_FUNCTION(
                            trace, eval_func(trace_eval).print_func<imathprint>(trace_print_imath));
                        ...
                        static void charpoly_print_imath(const ex& e, const ex& var,
                                                         const print_context& c)
                        {
                            c.s << "func charpoly( ";
                            e.print(c);
                        ...
                        REGISTER_FUNCTION(
                            charpoly,
                            eval_func(charpoly_eval).print_func<imathprint>(charpoly_print_imath));
                        ...
                        static ex solvematrix_eval(const ex& e, const ex& vars, const ex& rhs)
                        {
                            ...
                            MSG_INFO(3, "solvematrix: " << e << " for " << vars
                            ...
                            if (is_a<matrix>(e) && is_a<matrix>(vars) && is_a<matrix>(rhs))
                            {
                                ...
                                if (m.cols() == v.rows() && r.cols() == v.cols()
                                    && r.rows() == m.rows())
                                    return m.solve(v, r);
                            }

                            return Functionmanager::create_hard("solvematrix",
                                                                exprseq{ e, vars, rhs }, false);
                        }
                        ...
                        REGISTER_FUNCTION(invertmatrix,
                                          eval_func(invertmatrix_eval)
                        ...
                        REGISTER_FUNCTION(matrixrows,
                                          eval_func(matrixrows_eval)
                        ...
                        REGISTER_FUNCTION(matrixcols,
                                          eval_func(matrixcols_eval)
        ...
        REGISTER_FUNCTION(vmax, eval_func(vmax_eval).print_func<imathprint>(vmax_print_imath));
        ...
        REGISTER_FUNCTION(vmin, eval_func(vmin_eval).print_func<imathprint>(vmin_print_imath));
        ...
        REGISTER_FUNCTION(irem, eval_func(irem_eval).print_func<imathprint>(irem_print_imath));
        ...
        REGISTER_FUNCTION(iquo, eval_func(iquo_eval).print_func<imathprint>(iquo_print_imath));
        ...
        REGISTER_FUNCTION(numer, eval_func(numer_eval).print_func<imathprint>(numer_print_imath));
        ...
        REGISTER_FUNCTION(denom, eval_func(denom_eval).print_func<imathprint>(denom_print_imath));
        ...
        REGISTER_FUNCTION(
            diagmatrix, eval_func(diagmatrix_eval).print_func<imathprint>(diagmatrix_print_imath));
        ...
        REGISTER_FUNCTION(
            identmatrix,
            eval_func(identmatrix_eval).print_func<imathprint>(identmatrix_print_imath));
        ...
        REGISTER_FUNCTION(
            onesmatrix, eval_func(onesmatrix_eval).print_func<imathprint>(onesmatrix_print_imath));
        ...
        static ex submatrix_eval(const ex& e, const ex& r, const ex& nr, const ex& c, const ex& nc)
        {
            ...
            MSG_INFO(3, "submatrix: " << e << ", " << r << ", " << nr << ", " << c << ", " << nc
                                      << endline);

            if (is_a<matrix>(e) && is_a<numeric>(r) && is_a<numeric>(nr) && is_a<numeric>(c)
                && is_a<numeric>(nc))
            {
                const matrix& m = ex_to<matrix>(e);
                ...
                if (row.info(info_flags::posint) && nrows.info(info_flags::posint)
                    && col.info(info_flags::posint) && ncols.info(info_flags::posint))
                    return sub_matrix(m, row.to_int() - 1, nrows.to_int(), col.to_int() - 1,
                                      ncols.to_int());
            }
        ...
        REGISTER_FUNCTION(
            determinant,
            eval_func(determinant_eval).print_func<imathprint>(determinant_print_imath));
        ...
        REGISTER_FUNCTION(trace, eval_func(trace_eval).print_func<imathprint>(trace_print_imath));
        ...
        {
            // Immediately evaluate if arguments are matching matrices
            MSG_INFO(3, "solvematrix: " << e << " for " << vars << " with right-hand side " << rhs
                                        << endline);
        ...
        REGISTER_FUNCTION(
            invertmatrix,
            eval_func(invertmatrix_eval).print_func<imathprint>(invertmatrix_print_imath));
        ...
        REGISTER_FUNCTION(
            matrixrows, eval_func(matrixrows_eval).print_func<imathprint>(matrixrows_print_imath));
        ...
        REGISTER_FUNCTION(
            matrixcols, eval_func(matrixcols_eval).print_func<imathprint>(matrixcols_print_imath));
    }
