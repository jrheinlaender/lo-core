...
static ex mindex_eval(const ex& e, const ex& r, const ex& c)
{
    ...
    if (is_a<matrix>(e))
    {
        ...
        REGISTER_FUNCTION(mindex,
                          eval_func(mindex_eval).print_func<imathprint>(mindex_print_imath));
        ...
        static ex hadamard_eval(const ex& e1, const ex& e2, const ex& op)
        {
            ...
            MSG_INFO(1, "hadamard eval: "
                            << e1 << (h_op == h_product ? "*" : (h_op == h_division ? "/" : "^"))
                            << e2 << endline);
        ...
        static ex vecprod_eval(const ex& e1, const ex& e2)
        {
            ...
            if (is_a<matrix>(e1) && is_a<matrix>(e2))
            {
                ...
                if (!(((v1.cols() == 1) && (v2.cols() == 1) && (v1.rows() == v2.rows())
                       && (v1.rows() == 3))
                      || ((v1.rows() == 1) && (v2.rows() == 1) && (v1.cols() == v2.cols())
                          && (v1.cols() == 3))))
                    throw std::invalid_argument("Error: The vector product can only be calculated "
        ...
        static ex onesmatrix_eval(const ex& r, const ex& c)
        {
            ...
            REGISTER_FUNCTION(trace,
                              eval_func(trace_eval).print_func<imathprint>(trace_print_imath));
            ...
            REGISTER_FUNCTION(
                charpoly, eval_func(charpoly_eval).print_func<imathprint>(charpoly_print_imath));
            ...
            static ex solvematrix_eval(const ex& e, const ex& vars, const ex& rhs)
            {
                ...
                MSG_INFO(3, "solvematrix: " << e << " for " << vars << " with right-hand side "
                                            << rhs << endline);
            ...
            REGISTER_FUNCTION(
                matrixrows,
                eval_func(matrixrows_eval).print_func<imathprint>(matrixrows_print_imath));
            ...
            REGISTER_FUNCTION(
                matrixcols,
                eval_func(matrixcols_eval).print_func<imathprint>(matrixcols_print_imath));
        }
