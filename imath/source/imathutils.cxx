...
short ShowMessageBox(const Reference<XToolkit>& rToolkit, const Reference<XFrame>& rFrame,
                     const OUString& aTitle, const OUString& aMsgText, const unsigned width,
                     const long buttons)
{
    if (rFrame.is() && rToolkit.is())
    {
        ...
        aDescriptor.Bounds = com::sun::star::awt::Rectangle(
            0, 0, width, 200); // the Height attribute seems to have no effect
        ...
        if (xPeer.is())
        {
            ...
            Reference<XMessageBox> xMessageBox = xMsgBoxFactory->createMessageBox(
                xPeer, ::com::sun::star::awt::MessageBoxType_MESSAGEBOX, buttons, aTitle, aMsgText);
#else
            Reference<XMessageBox> xMessageBox = xMsgBoxFactory->createMessageBox(
                xPeer, com::sun::star::awt::Rectangle(), OU("errorbox"), buttons, aTitle, aMsgText);
#endif
...
Reference<XDialog> createBasicDialog(const Reference<XComponentContext>& xCC,
                                     const Reference<XToolkit>& xToolkit,
                                     const Reference<XFrame>& xFrame, const OUString& DialogURL,
                                     const Reference<XInterface>& xHandler)
{
    try
    {
        ...
        if (xModel.is())
        {
            ...
            xDialogProvider = Reference<XDialogProvider2>(
                xMCF->createInstanceWithArgumentsAndContext(OU("com.sun.star.awt.DialogProvider2"),
                                                            args, xCC),
                UNO_QUERY_THROW);
        }
        else
        {
            xDialogProvider = Reference<XDialogProvider2>(
                xMCF->createInstanceWithContext(OU("com.sun.star.awt.DialogProvider2"), xCC),
                UNO_QUERY_THROW);
        }
        if (xHandler.is())
...
Reference<XInterface> getUserSelection(const Reference<XController>& xCtrl)
{
    ...
    Reference<XIndexAccess> xIndexAccess(oSelection,
                                         UNO_QUERY); // UNO_QUERY_THROW is not appropriate here!
    Reference<XInterface> oIndex;

    if (xIndexAccess.is())
    { // The selection is a container
    ...
    if (!oIndex.is())
        throw RuntimeException(OU("Invalid selection"), oIndex);
    return oIndex;
} // getUserSelection()
...
Sequence<Reference<XInterface>> getUserSelections(const Reference<XController>& xCtrl)
{
    ...
    { // The selection is a container
        ...
        for (int i = 0; i < xIndexAccess->getCount(); i++)
        {
            ...
            Reference<XInterface> oI(Index, UNO_QUERY_THROW);
            if (!oI.is())
                throw RuntimeException(OU("Invalid selection"), oSelection);
            pResult[i] = oI;
        }
                        ...
                        const bool strict)
{
    try
    {
        ...
        mFormula
            = Reference<XComponent>(oSelection, UNO_QUERY); // UNO_QUERY_THROW makes .is() redundant
...
Reference<XText> getDocumentText(const Reference<XController>& xCtrl,
                                 Reference<XTextCursor>& xModelCursor)
{
    ...
    Reference<XTextContent> xTextContent(oSelection,
                                         UNO_QUERY); // UNO_QUERY_THROW is not appropriate here
...
Reference<XComponent> createObject(const Reference<XModel>& xModel, const OUString& clsid)
{
    if (docType(xModel).equalsAscii("TextDocument"))
    {
        ...
        try
        {
            xObj = Reference<XComponent>(
                xDocMSF->createInstance(OU("com.sun.star.text.TextEmbeddedObject")),
                UNO_QUERY_THROW);
        }
        catch (Exception& e)
        {
...
Reference<XComponent> insertObject(const Reference<XModel>& xModel, const OUString& clsid)
{
    ...
    if (docType(xModel).equalsAscii("TextDocument"))
    {
        ...
        {
            (void)e; /* nothing was selected */
        }

        // Insert the object
        Reference<XTextRange> xRange(xModelCursor, UNO_QUERY_THROW);
        xDocumentText->insertTextContent(xRange, Reference<XTextContent>(xObject, UNO_QUERY_THROW),
                                         true);
    }
    else if (docType(xModel).equalsAscii("Presentation"))
    {
        ...
        {
            Any aSize = fPS->getPropertyValue(OU(
                "OriginalSize")); // Note: OriginalSize is always zero before the Shape is added to xShapes
            aSize >>= size;
                       ...
                       const sal_Int32 idx)
    {
        ...
        pSequences[0]
            = Reference<XLabeledDataSequence>(templateSeq->createClone(), UNO_QUERY_THROW);
        pSequences[1]
            = Reference<XLabeledDataSequence>(templateSeq->createClone(), UNO_QUERY_THROW);
                  ...
                  const unsigned iseries)
{
    ...
    for (unsigned r = 0; r < rows; ++r)
    {
        ...
        for (unsigned r = 0; r < rows; ++r)
        {
            ...
            {
                pNewPoint[iseries - 1]
                    = forceDouble(expression(newx(r, 0)), not_a_number); // X-values
                pNewPoint[iseries] = forceDouble(expression(newy(r, 0)), not_a_number); // Y-values
            }
    ...
    Reference<XComponent> getChartObjectByName(const Reference<XModel>& xModel,
    ...
    Reference<com::sun::star::chart2::XChartDocument> getChartDoc(const Reference<XModel>& xModel,
                                                                  const OUString& cName)
    {
    ...
    Reference<XChartDataArray> getChartDataArray(
        const Reference<com::sun::star::chart2::XChartDocument>& cDoc)
    {
...
bool checkTextmodeFormula(const Reference<XTextContent>&(formula))
{
    ...
    if (!xCursor->getString().trim().isEmpty())
