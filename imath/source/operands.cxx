...
ex mul_ops(const ex& op1, const ex& op2) { return (op1 * op2); } //mul_ops()

ex add_ops(const ex& op1, const ex& op2) { return (op1 + op2); } //add_ops()
...
operands::operands(const ex& ops_type)
{
    ...
    symbols = constants = units = functions = integrals = differentials = derivatives = adds = muls
        = powers = matrices = others = coefficient = ops_type;
...
operands::operands(const operands& other)
    : type(other.type)
...
void operands::split_ex(const expression& e, operands& o1, operands& o2)
{
    ...
    MSG_INFO(3,
             "Splitting expression " << e << ", number of operands: " << (long)e.nops() << endline);
...
std::string operands::pattern() const
{
    // Sign
    std::string result
        = (is_negex(coefficient)
               ? "-"
               : ""); // Note that .info(info_flags::negative) is not sufficient, e.g. for -1/2 sqrt(-3) or -I
...
{
    if (type == GINAC_ADD)
        return symbols + constants + units + functions + integrals + differentials + derivatives
               + adds + muls + powers + matrices + others + coefficient;
    else
        return symbols * constants * units * functions * integrals * differentials * derivatives
               * adds * muls * powers * matrices * others * coefficient;
}
