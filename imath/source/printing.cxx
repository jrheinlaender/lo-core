        ...
        for (size_t i = 0; i < itemVector.size(); ++i)
        {
            ...
            else if (item.substr(0, 2) == "$*")
            {
                ...
                if (!subMatch.empty())
                {
                    ...
                    printSortedAdd(collectSubmatch(subMatch, is_positive ? posops : negops), c,
                                   sign);
                    firstItem = false;
                }
            }
            else if (item[0] == '$')
            {
                ...
                for (size_t m = 0; m < subMatch.size(); ++m)
                {
                    ...
                    auto result
                        = printAddItem(std::string(1, static_cast<char>(std::tolower(subMatch[m]))),
...
static const std::vector<std::pair<std::regex, std::string>> mulPrintFormats = {
    ...
    { std::regex("([nNcCxX]+)([ef]?)r/([ef])"),
      "$0 { frac{ alignc $1 } over { alignc $2 frac} } r" },
    { std::regex("([nNcCxX]+)([ef]?)r/([nNcCxX]+)([ef])"), "{ frac{ alignc $0 } over { alignc $2 "
    ...
    { std::regex("/(.+)"),
      "{ frac{ alignc 1 }  over { alignc $0 frac} }" }, // Catchall for everything that remains
