...
imathprint::imathprint(std::ostream& os, optionmap* popt)
    : print_dflt(os)
    , poptions(popt)
{
    ata = new bool(false);
...
imathprint::imathprint(std::ostream& os, const imathprint& c)
    : print_dflt(os, c.options)
    , poptions(c.poptions)
{
    ata = new bool(c.add_turn_around());
}

imathprint::~imathprint() { delete ata; }
...
std::string ex_get_name(const ex& e)
{
    ...
    {
        if ((e.nops() == 2) && is_a<numeric>(e.op(1))
            && (is_a<symbol>(e.op(0)) || is_a<constant>(e.op(0))))
        {
...
std::string printAddItem(const std::string& item, const operands& ops, const imathprint& c,
                         const char op)
{
    ...
    else if (item == "p")
    {
        ...
        else if (is_a<mul>(p))
        {
            ...
            if (resultstring[0] == '-')
                return resultstring.substr(
                    1); // Remove - that was multiplied into the expression. Caller will supply leading minus sign
            else
            ...
            for (auto a = p.begin(); a != p.end(); ++a)
            {
                ...
                if (a == p.begin())
                {
                    if (innerresultstring[0] == '-')
                        result.s << innerresultstring.substr(
                            1); // Remove - that was multiplied into the expression. Caller will supply leading minus sign
                    else
                        result.s
                            << "-" + innerresultstring; // Caller will detect leading double minus sign
                }
                else
                {
                    if (innerresultstring[0] == '-')
                        result.s
                            << " "
                            << innerresultstring; // Use - that was multiplied into the expression
                    else
    ...
    if (check_polynomial(a, sym))
    {
        ...
        for (int i = a.degree(sym); i >= a.ldegree(sym); --i)
        {
            ...
            for (const auto & [ pat, format ] : addPrintFormats)
            {
                ...
                bool firstItem(
                    true); // Note: Checking for i > 0 is not sufficient, since there might be empty submatches at the beginning of the pattern
                char sign('+');
                ...
                for (size_t i = 0; i < itemVector.size(); ++i)
                {
                    ...
                    else if (item.substr(0, 2) == "$*")
                    {
                        ...
                        if (!subMatch.empty())
                        {
                            ...
                            printSortedAdd(collectSubmatch(subMatch, is_positive ? posops : negops),
                                           c, sign);
                            firstItem = false;
                        }
                    }
                    else if (item[0] == '$')
                    {
                        ...
                        for (size_t m = 0; m < subMatch.size(); ++m)
                        {
                            ...
                            auto result = printAddItem(
                                std::string(1, static_cast<char>(std::tolower(subMatch[m]))),
                                is_positive ? posops : negops, c, sign);

                            if (result[0] == '-')
                            { // Note: Result can only start with - if sign == '-'
                                if (!firstItem)
                            ...
                            if (i >= 0)
                            {
                                ...
                                {
                                    if (is_a<add>(coeff) && ((i != 0) || changed_sign))
                                        c.s << "(";
                ...
                for (const auto & [ pat, format ] : addPrintFormats)
                {
                    ...
                    {
                        item = itemVector
                            [i]; // Need to iterate by index to be able to test for i > 0
                        ...
                        else if (item.substr(0, 2) == "$*")
                        {
                            ...
                            if (!subMatch.empty())
                            {
                                ...
                                // Note: No attempt to turn around adds here, since priority is on the ordering of the operands
                                printSortedAdd(
                                    collectSubmatch(subMatch, is_positive ? posops : negops), c,
                                    sign);
                                firstItem = false;
                            }
                        }
                        else if (item[0] == '$')
                        {
                            ...
                            const std::string& subMatch = subMatches[subMatchIdx++].str();
                            MSG_INFO(1,
                                     "Printing ordered submatch '" << subMatch << "'" << endline);
                            for (size_t m = 0; m < subMatch.size(); ++m)
                            {
                                // Note: printAddItem tries to eliminate a negative sign by turning around one add inside a product
                                auto result = printAddItem(
                        ...
                        {
                            auto result
                                = printAddItem(item, is_positive ? posops : negops, c, sign);
            ...
            void imathprint_constant(const constant& cn, const imathprint& c, unsigned level)
            {
                ...
                { std::regex("n([uU]?)/[uU]"), "n { frac{ alignc $0 } over { alignc u frac} }" },
                    { std::regex("n([uU]?)([^/]+)/[uU]"),
                      "n { frac{ alignc $0 } over { alignc u frac} } $1" },
                    { std::regex("n([uU]?)([^/]*)/[uU]([^/]+)"), "n { frac{ alignc $0 } over { "
                                                                 "alignc u frac} } { frac{ alignc "
                                                                 "$1 } over { alignc $2 } }" },
                    ...
                    { std::regex("([ef]?)([rRdD]*)/([ef])([rRdD]+)"),
                      "{ frac{ alignc $0 } over { alignc $2 frac} } { frac{ alignc $1 } over { "
                      "alignc $3 frac} }" },
                    { std::regex("([nNcCxX]+)([ef]?)([rRdD]+)/([ef])"),
                      "$0 { frac{ alignc $1 } over { alignc $3 frac} } $2" },
                    { std::regex("([nNcCxX]+)([ef]?)([rRdD]*)/([ef])([rRdD]+)"),
                      "$0 { frac{ alignc $1 } over { alignc $3 frac} } { frac{ alignc $2 } over { "
                      "alignc $4 frac} }" },
                    { std::regex("([nNcCxX]+)([ef]?)([rRdD]+)/([nNcCxX]+)([ef])"),
                      "{ frac{ alignc $0 } over { alignc $3 frac} } { frac{ alignc $1 } over { "
                      "alignc $4 frac} } $2" },
                    { std::regex("([nNcCxX]+)([ef]?)([rRdD]*)/([nNcCxX]+)([ef])([rRdD]+)"),
                      "{ frac{ alignc $0 } over { alignc $3 frac} } { frac{ alignc $1 } over { "
                      "alignc $4 frac} } { frac{ alignc $2 } over { alignc $5 frac} }" },
                    ...
                    { std::regex("([nNcCxX]+)([^/nNcCxX]+)/([nNcCxX]+)"),
                      "{ frac{ alignc $0 } over { alignc $2 frac} } $1" },
                    { std::regex("a/([nNcXxX]+)"),
                      "{ frac{ alignc 1 }  over { alignc $0 frac} } a" },
            ...
            for (const auto & [ pat, format ] : mulPrintFormats)
            {
                ...
                for (size_t i = 0; i < itemVector.size(); ++i)
                {
                    ...
                    else if (item[0] == '$')
                    {
                        ...
                        void imathprint_exprseq(const exprseq& es, const imathprint& c,
                                                unsigned level)
                        {
                        ...
                        void imathprint_function(const function& f, const imathprint& c,
                                                 unsigned level)
                        {
                            MSG_INFO(4,
                                     "Inside imathprint_function()"
                                         << endline); // Important for printing hard-coded functions
                            (void)level;
                        ...
                        void imathprint_matrix(const matrix& m, const imathprint& c, unsigned level)
                        {
                            ...
                            if (m.cols() == 1)
                            { // stack{} is only useable for matrices with one single column
                                c.s << "(alignc STACK{";
                                for (unsigned r = 0; r < m.rows();)
                                {
                                    m(r, 0).print(
                                        c,
                                        0); // Level > 0 lets adds print brackets around themselves
                                    if (++r != m.rows())
                                        c.s << " # ";
                                }
                            }
                            ...
                            bool printMulItem(const std::string& item, const operands& ops,
                                              imathprint& c, unsigned level, bool& turn_around)
                            {
                                bool handled = true;
                            ...
                            static const std::vector<std::pair<std::regex, std::string>>
                                mulPrintFormats = {
                                    ...
                                    if (item
                                        == "n"){ printSortedMul(ops.get_coefficient(), c, level);
                            }
                            ...
                            else if (item == "r")
                            {
                                ...
                                {
                                    std::regex("([^/]+)"), "$0"
                                }, // Catchall for muls without fractions. Prints operands in the default order
                                    ...
                                    { std::regex("([nNcCxX]+)([ef]?)([rRdD]+)/([nNcCxX]+)([ef])"),
                                      "{ frac{ alignc $0 } over { alignc $3 frac} } { frac{ alignc "
                                      "$1 } over { "
                                      "alignc $4 "
                                      "frac} "
                                      "} "
                                    ...
                                    // Catchall for everything that remains
                                    { std::regex("/(.+)"),
                                      "{ frac{ alignc 1 }  over { alignc $0 frac} }" },
                                {
                                    std::regex("([^/]+)/(.+)"),
                                        "{ frac{ alignc $0 } over { alignc $1 frac} }"
                                }
                            };
                        }
                        ...
                        void imathprint_mul(const mul& m, const imathprint& c, unsigned level)
                        {
                            ...
                            operands numer(GINAC_MUL), denom(GINAC_MUL), tempn(GINAC_MUL),
                                tempd(GINAC_MUL), temp(GINAC_MUL);
                            operands::split_ex(m, numer, denom);
                            ...
                            for (const auto & [ pat, format ] : mulPrintFormats)
                            {
                                ...
                                void imathprint_mul(const mul& m, const imathprint& c,
                                                    unsigned level)
                                {
                                    MSG_INFO(2, "imathprint_mul() for " << m << endline);
                                    // Note: The level parameter is ignored and used for other purposes
                                    ...
                                    if (!denom.is_trivial())
                                        pattern += "/" + denom.pattern();
                                    MSG_INFO(2, "Multiplicative pattern '" << pattern << "'"
                                                                           << endline);
                                    ...
                                    for (const auto & [ pat, format ] : mulPrintFormats)
                                    {
                                        ...
                                        for (size_t i = 0; i < itemVector.size(); ++i)
                                        {
                                            ...
                                            else if (item[0] == '$')
                                            {
                                                ...
                                                const std::string& subMatch
                                                    = subMatches[subMatchIdx++].str();
                                                MSG_INFO(2, "Printing submatch '" << subMatch << "'"
                                                                                  << endline);
                                                if (subMatch.empty())
                                                    result.s
                                                        << "1"; // Empty matches by definition print 1

                                                for (size_t m = 0; m < subMatch.size(); ++m)
                                                {
                                                    if (subMatch.size() != 1 && subMatch[m] == 'a')
                                                        printMulItem(
                                                            "a", is_numer ? numer : denom, result,
                                                            1,
                                                            negative); // A single add with preceding or following other operands must be bracketed
                                                    else
                                                        printMulItem(
                                                            std::string(
                                                                1, static_cast<char>(
                                                                       std::tolower(subMatch[m]))),
                                                            is_numer ? numer : denom, result, 0,
                                                            negative);
                                                    result.s << " ";
                                                }
                                                else if (item == "over")
                                                ...
                                                {
                                                    if (!printMulItem(
                                                            item, is_numer ? numer : denom, result,
                                                            item == "a" ? 1 : 0, negative))
                                    ...
                                    void imathprint_ncmul(const ncmul& m, const imathprint& c,
                                                          unsigned level)
                                    {
                                    ...
                                    std::string roundNumber(const std::string& number,
                                                            const int pos, bool& overflow)
                                    {
                                        MSG_INFO(4,
                                                 "Rounding " << number << " to " << pos << endline);
                                ...
                                if (is_a<func>(basis)
                                    && (ex_to<func>(basis).is_trig()
                                        || ex_to<func>(basis).is_pure()))
                                {
                                ...
                                else
                                {
                                    ...
                                    if (is_a<func>(basis))
                                        bracket = !ex_to<func>(basis).is_nobracket();
                                             ...
                                             && (ex_to<differential>(basis).is_numerator()
                                                 || !ex_to<differential>(basis)
                                                         .get_grade()
                                                         .is_equal(_ex1)))
                                        bracket = true;
                                    else if (is_a<expairseq>(basis) || is_a<power>(basis)
                                             || is_a<exderivative>(basis)
                                             || (is_a<numeric>(basis)
                                                 && (basis.info(info_flags::negative)
                                                     || basis.is_equal(_ex_1 * I))))
                                        bracket = true;

                                    void imathprint_real(const numeric& num, const imathprint& c)
                                    {
                                        unsigned precision
                                            = (*c.poptions)[o_precision].value.uinteger;
                                        bool fixeddigits
                                            = (*c.poptions)[o_fixeddigits].value.boolean;
                                        ...
                                        { // print float
                                            ...
                                            numstream
                                                << num; // Use standard printing routine of numeric
                                            ...
                                            MSG_INFO(
                                                4, ", Forced exponent "
                                                       << (*c.poptions)[o_exponent].value.integer);
                                            MSG_INFO(
                                                4,
                                                ", High limit "
                                                    << (*c.poptions)[o_highsclimit].value.integer);
                                            MSG_INFO(
                                                4, ", Low limit "
                                                       << (*c.poptions)[o_lowsclimit].value.integer
                                                       << endline);

                                            if (fixeddigits && (precision == 0))
                                                throw std::runtime_error(
                                                    "It is not possible to print a number with "
                                                    "zero "
                                                    "significant "
                                            ...
                                            bool scientific
                                                = ((exponent
                                                    > (*c.poptions)[o_highsclimit].value.integer)
                                                   || (exponent <= -(*c.poptions)[o_lowsclimit]
                                                                        .value.integer));
                                            if (fixedexponent != 0)
                                            {
                                            ...
                                            {
                                                move
                                                    = exponent; // Eliminate the need for an exponent;
                                            }
                                            ...
                                            if (fixeddigits)
                                                ...
                                                number = roundNumber(number, precision + move,
                                                                     overflow);
                                            if (overflow)
                                            {
                                                if (!scientific)
                                            ...
                                            if (fixeddigits)
                                                number.erase(number.find_last_not_of('0')
                                                             + 1); // Remove trailing zeros
                                            MSG_INFO(4, "Rounded number='"
                                                            << number << "', move=" << move
                                                            << ", exponent=" << exponent
                                                            << endline);
                                            ...
                                            if (move < 0)
                                            {
                                                ...
                                                if (!fixeddigits)
                                                {
                                                    if ((number.size() == 0)
                                                        ...
                                                        zeros = std::string(
                                                            precision,
                                                            '0'); // Avoid too many trailing zeros

                                                    // Borderline case...
                                                    if (precision > number.size() - move)
                                                        number += std::string(
                                                            precision - (number.size() - move),
                                                            '0'); // Add some trailing zeros
                                                }

                                                number = "0" + imathprint::decimalpoint + zeros
                                                         + number;
                                            }
                                            else if (move > (int)number.size())
                                            {
                                                number = number
                                                         + std::string(move - number.size(), '0');
                                                if (!fixeddigits)
                                                    number
                                                        = number + imathprint::decimalpoint
                                                          + std::string(precision,
                                                                        '0'); // Add trailing zeros
                                            }
                                            else
                                            {
                                                if (!fixeddigits
                                                    && (number.size() <= precision + move))
                                                    number = number
                                                             + std::string(precision + move
                                                                               - number.size(),
                                                                           '0');

                                                if (move == 0)
                                                    number
                                                        = "0" + imathprint::decimalpoint + number;
                                                else if (
                                                    move
                                                    != (int)number
                                                           .size()) // Avoid trailing decimal point
                                                    number.insert(move, imathprint::decimalpoint);
                                            }
                                            MSG_INFO(4,
                                                     "Number after moving: " << number << endline);

                                            int remainingexponent = exponent - move;
                                            if (remainingexponent != 0)
                                                number = number + " cdot 10^"
                                                         + std::to_string(remainingexponent);
                                            if (negative)
                                                number = "-" + number;
                                    ...
                                    void imathprint_numeric(const numeric& n, const imathprint& c,
                                                            unsigned level)
                                    {
                                    ...
                                    void imathprint_power(const power& p, const imathprint& c,
                                                          unsigned level)
                                    {
                                        ...
                                        if (is_a<numeric>(expon)
                                            && (ex_to<numeric>(expon)).is_rational())
                                    ...
                                    void imathprint_relational(const relational& r,
                                                               const imathprint& c, unsigned level)
                                    {
                                    ...
                                    void imathprint_symbol(const symbol& s, const imathprint& c,
                                                           unsigned level)
                                    {
                                        ...
                                        c.s << (pos == std::string::npos ? name
                                                                         : name.substr(pos + 1));
                                    }

                                    void imathprint_wildcard(const wildcard& w, const imathprint& c,
                                                             unsigned level)
                                    {
                                    ...
                                    void imathprint_generic(const basic& b, const imathprint& c,
                                                            unsigned level)
                                    {
                                        MSG_INFO(0, "!!! imathprint_generic() for '" << ex(b) << "'"
                                                                                     << endline);
                                        (void)level;
