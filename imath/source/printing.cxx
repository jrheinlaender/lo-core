...
void imathprint_add(const add& a, const imathprint& c, unsigned level)
{
    ...
    if (check_polynomial(a, sym))
    {
        ...
        for (int i = a.degree(sym); i >= a.ldegree(sym); --i)
        {
            ...
            {
                MSG_INFO(4, "Inside imathprint_function()"
            ...
            void imathprint_matrix(const matrix& m, const imathprint& c, unsigned level)
            {
                ...
                else
                {
                    ...
                    for (unsigned r = 0; r < m.rows();)
                    {
                        ...
                        {
                            m(r, col).print(
                                c, 0); // Level > 0 lets adds print brackets around themselves
                            if (++col != m.cols())
                                c.s << " # ";
                        }
                        if (++r != m.rows())
                ...
                bool printMulItem(const std::string& item, const operands& ops, imathprint& c,
                                  unsigned level, bool& turn_around)
                {
                    bool handled = true;
            ...
            void imathprint_mul(const mul& m, const imathprint& c, unsigned level)
            {
                ...
                for (const auto & [ pat, format ] : mulPrintFormats)
                {
                    ...
                    void imathprint_mul(const mul& m, const imathprint& c, unsigned level)
                    {
                        ...
                        for (const auto & [ pat, format ] : mulPrintFormats)
                        {
                            ...
                            for (size_t i = 0; i < itemVector.size(); ++i)
                            {
                                ...
                                else if (item[0] == '$')
                                {
                                    ...
                                    const std::string& subMatch = subMatches[subMatchIdx++].str();
                                    MSG_INFO(2,
                                             "Printing submatch '" << subMatch << "'" << endline);
                                    if (subMatch.empty())
                                    ...
                                    else if (item[0] == '$')
                                    {
                                        ...
                                        MSG_INFO(1, "Printing submatch '" << subMatch << "'"
                                                                          << endline);
                                        if (subMatch.empty())
                                            result.s << "1"; // Empty matches by definition print 1
                                    ...
                                    {
                                        if (!printMulItem(item, is_numer ? numer : denom, result,
                                                          item == "a" ? 1 : 0, negative))
                        ...
                        std::string roundNumber(const std::string& number, const int pos,
                                                bool& overflow)
                        {
                    ...
                    if (is_a<func>(basis)
                        && (ex_to<func>(basis).is_trig() || ex_to<func>(basis).is_pure()))
                    {
                    ...
                    else
                    {
                        ...
                        void imathprint_real(const numeric& num, const imathprint& c)
                        {
                            ...
                            { // print float
                                ...
                                bool scientific
                                    = ((exponent > (*c.poptions)[o_highsclimit].value.integer)
                                ...
                                if (move < 0)
                                {
                                    ...
                                    if (!fixeddigits)
                                    {
                                        if ((number.size() == 0) && ((int)precision < -move))
                                            zeros = std::string(
                                                precision, '0'); // Avoid too many trailing zeros

                                        // Borderline case...
                                        if (precision > number.size() - move)
                                            number
                                                += std::string(precision - (number.size() - move),
                                ...
                                else
                                {
                                    if (!fixeddigits && (number.size() <= precision + move))
                                        number
                                            = number
                                              + std::string(precision + move - number.size(), '0');

                                    if (move == 0)
                                        number = "0" + imathprint::decimalpoint + number;
                                    else if (move
                                             != (int)number.size()) // Avoid trailing decimal point
                                ...
                                if (remainingexponent != 0)
                                    number
                                        = number + " cdot 10^" + std::to_string(remainingexponent);
                                if (negative)
                                    number = "-" + number;
                            ...
                            { // exponent is a rational number
                                ...
                                { // exponent is a positive fraction
                                    ...
                                    if (!exponent.numer().is_equal(*_num1_p))
                                    {
                                        ...
                                        {
                                            bool bracket
                                                = is_a<expairseq>(basis) || is_a<func>(basis)
                                                  || is_a<power>(basis) || is_a<exderivative>(basis)
                            ...
                            else
                            {
                                ...
                                if (is_a<func>(basis))
                                    bracket = !ex_to<func>(basis).is_nobracket();
                                         ...
                                         && (ex_to<differential>(basis).is_numerator()
                                             || !ex_to<differential>(basis).get_grade().is_equal(
                                                    _ex1)))
                                    bracket = true;
                                else if (is_a<expairseq>(basis) || is_a<power>(basis)
                                         || is_a<exderivative>(basis)
                        ...
                        void imathprint_relational(const relational& r, const imathprint& c,
                                                   unsigned level)
                        {
                        ...
                        {
                            MSG_INFO(0,
                                     "!!! imathprint_generic() for '" << ex(b) << "'" << endline);
                            (void)level;
