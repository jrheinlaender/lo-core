...
imathprint::imathprint(std::ostream& os, const imathprint& c)
    ...
    , poptions(c.poptions)
    , add_turn_around(c.add_turn_around)
{
}
...
bool check_has_negop(const ex& adds)
{
    ...
    if (is_a<add>(adds))
    { // There is only one add
        if (adds.nops() != 2)
            return false; // We don't touch things like a - b + c because we want alphabetical ordering
...
void print_smath_ops(const ex& e, const std::string& sym, const imathprint& c, const bool toplevel,
                     size_t& opnum, const bool add_turn_around = false)
{
    (void)toplevel;
    ...
    void print_smath_mul(const operands& m, const imathprint& c, const size_t ops,
                         const bool add_turn_around = false)
    {
        ...
        size_t opnum = ops;
        if (!is_equal_int(ex_to<numeric>(m.get_coefficient()), 1,
        ...
        {
            for (const auto& i :
                 (is_a<mul>(m.get_powers()) ? m.get_powers() : lst{ m.get_powers() }))
            {
                if (is_a<numeric>(get_basis(ex_to<power>(i)))
                    && is_a<numeric>(get_exp(ex_to<power>(i))))
                {
...
void imathprint_add(const add& a, const imathprint& c, unsigned level)
{
    ...
    if ((a.nops() == 2) && (is_negex(a.op(0)) != is_negex(a.op(1))))
    { // Only two operands, only one is negative
        ...
        for (std::vector<ex>::const_iterator i = ops.begin(); i != ops.end(); i++)
        {
            if (i != ops.begin())
            {
                if (is_negex(*i))
                {
                    ...
                    i->print(tprint,
                             level + 1); // This will try to get rid of negative coefficients
    ...
    { // Everything else is sorted alphabetically if possible
...
void imathprint_mul(const mul& m, const imathprint& c, unsigned level)
{
    ...
    if (numer.get_coefficient().info(info_flags::negative))
    ...
    bool n_one_symbol
        = ((is_a<symbol>(nsymbols) || is_a<constant>(nconstants)) && coeff_nd.op(0).is_equal(_ex1)
           && numer.get_adds().is_equal(_ex1) && numer.check_functions(_ex1)
           ...
           && numer.get_differentials().is_equal(_ex1) && numer.get_derivatives().is_equal(_ex1));
    ...
    if (!n_one_symbol)
    {
        ...
        bool has_quantity
            = (!coefficient.is_equal(_ex1) && (!n_units.is_equal(_ex1) || !d_units.is_equal(_ex1)));

        if (!has_quantity
            && (!coefficient.is_equal(_ex1) && d_one_symbol && numer.check_symbols(_ex1)))
        { //Prevent things like 2 1/x , make them 2/x
            c.enter_fraction();
            c.s << "{{alignc ";
            (coeff_nd.op(0) * nconstants).print(c, level + 1);
            c.s << "} over {alignc ";
            if (!coeff_nd.op(1).is_equal(_ex1))
                coeff_nd.op(1).print(c, level + 1);
        ...
        else
        {
            ...
            if (!imag(ex_to<numeric>(coefficient)).is_zero()
                && !real(ex_to<numeric>(coefficient)).is_zero())
                c.s << "("; // put complex expression into brackets)
        ...
        if (!(n_units.is_equal(_ex1) && d_units.is_equal(_ex1)))
        {
            ...
            if (opnum != 0)
                opnum++; // The quantity counts as a single operand
        }
        ...
        numer.exclude(
            numer.get_coefficient()); // The coefficient and units have already been printed
    ...
    else
    {
        ...
        numer.include(
            tempcoeff.denom()); // This has the effect of removing the denominator part ...
    ...
    if (d_one_symbol && !nsymbols.is_equal(_ex1))
    { // There is only one symbol in the denominator
    ...
    else
    {
        ...
        MSG_INFO(3, "Remaining incomplete differentials: " << posdiffs << " / " << negdiffs
                                                           << endline);
...
void print_ncmul_fraction(const expression& n, const expression& d, const imathprint& c,
                          unsigned level)
{
