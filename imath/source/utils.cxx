    ...
    for (const auto& i : unitlist)
    {
        ...
        { // handles evalf()'ed cases for Pi = 180\degree
            ...
            if (is_a<equation>(e))
            {
                ...
                result
                    = equation(c_to_unit(eq.lhs()), c_to_unit(eq.rhs()), eq.getop(), eq.getmod());
            }
            else
            {
...
ex expand_real_powers::operator()(const ex& e)
{
    ...
    {
        MSG_INFO(2,
                 "Numerically evaluating nth roots to real values where possible " << e << endline);
    ...
    {
        MSG_INFO(3,
                 "Assigning diff_fraction from " << other.numer << "/" << other.denom << endline);
        numer = other.numer;
        remaining_grade = other.remaining_grade;
        diffpower = other.diffpower;
        denom = other.denom;
                          ...
                          const bool matchparents)
    {
        ...
        for (auto& l : dlist)
        {
            ...
            else
            {
                ...
                MSG_INFO(2, diffarg << " in numerator contains " << diffvar << " of denominator: "
                                    << (match ? "true" : "false") << endline);
            }
                ...
                if (!p_quot.info(info_flags::real))
                {
                        ...
                        || l.diffpower.has(g_quot))
                    {
                        ...
                        MSG_INFO(2, "Remaining numerator: "
                                        << pow(remainder.numer, remainder.diffpower) << endline);
        ...
        if (is_a<mul>(e))
        {
            ...
            if (g >= numeric_to_int(ex_to<numeric>(l.remaining_grade)))
            {
                MSG_INFO(2, "Consuming all remaining " << l.remaining_grade
                                                       << " grades of the numerator" << endline);
                l.denom *= dynallocate<differential>(diffvar, d.is_partial(), l.remaining_grade,
                                                     diffarg, false);
                restpower
                    -= l.remaining_grade
            ...
            else if (g > 0)
            {
                ...
                l.remaining_grade -= g;
                restpower
                    -= g * l.diffpower; // This cannot be zero since that case was already handled!
            } // otherwise no match (0 < p_quot < 1)
        }
    }
        ...
        for (const auto& m : mapped_e)
        {
            ...
            else if (is_a<power>(m))
            {
                ...
                for (auto& l : sorteddiffs)
                {
                    ...
                    {
                        result *= dynallocate<power>(dynallocate<exderivative>(l.numer, l.denom),
                                                     l.diffpower);
                    }
                }
