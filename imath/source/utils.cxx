                      ...
                      const bool matchparents)
{
    ...
    for (auto& l : dlist)
    {
        MSG_INFO(2, "Testing numerator " << pow(l.numer, l.diffpower) << " with remaining grade "
        ...
        if (matchparents)
        {
            ...
            MSG_INFO(2, "Parent of denominator matches numerator: " << (match ? "true" : "false")
                                                                    << endline);
        }
        else
        {
            ...
            MSG_INFO(2, diffarg << " in numerator contains " << diffvar
                                << " of denominator: " << (match ? "true" : "false") << endline);
        }
        ...
        if (match)
        {
            ...
            if (!p_quot.info(info_flags::real))
            {
                ...
                {
                    MSG_INFO(2,
                             "Splitting power of numerator and consuming denominator" << endline);
                    dlist.emplace_back(l);
                    diff_fraction& remainder = dlist.back();
                    remainder.diffpower = l.diffpower - g_quot;
                    MSG_INFO(2, "Remaining numerator: " << pow(remainder.numer, remainder.diffpower)
                                                        << endline);
            ...
            int g
                = numeric_to_int(ex_to<numeric>(p_quot)); // Find integer part of consumable grades
    ...
    if (is_a<mul>(e))
    {
        ...
        if (is_a<mul>(posdiffs))
        {
            for (const auto& d : posdiffs)
            {
                ...
                df.remaining_grade
                    = df.numer.get_grade(); // extract numeric grade for later matching
            }
        }
        else
        {
        ...
        MSG_INFO(2, "Negative differentials left-over after matching by parent: " << negrest2
                                                                                  << endline);
        ex negrest = match_negdiffs(sorteddiffs, negrest2,
                                    false); // Match by diffvar of the negative differential
        MSG_INFO(2, "Negative differentials left-over after matching by diffvar: " << negrest
                                                                                   << endline);
        ...
        for (auto& l : sorteddiffs)
        {
            ...
            {
                result
                    *= dynallocate<power>(dynallocate<exderivative>(l.numer, l.denom), l.diffpower);
            }
        }
