...
public:
  ...
  **/
    equation(const expression& le, const expression& ri, const operators op = equal,
             const expression& m = _expr0);
  ...
  **/
    inline equation& setop(const operators op)
    {
        o = op;
        return *this;
    }
  ...
  **/
    expression diff(const ex& var, const expression& nth = _expr1,
                    const bool toplevel = false) const;
  ...
  @exception logic_error(Can only differentiate with respect to a symbol or pure function)
  **/
    expression pdiff(const ex& var, const expression& nth = _expr1,
                     const bool toplevel = false) const;
  ...
  **/
    ...
    expression integrate(const ex& lvar, const extsymbol& l_integration_constant, const ex& rvar,
                         const extsymbol& r_integration_constant) const;
    expression integrate(const ex& lvar, const ex& l_lowerbound, const ex& l_upperbound,
...
protected:
    ...
    virtual ex derivative(const symbol& s) const
        override; // TODO Will this cause trouble with extsymbols that do not match the symbol?
