...
public:
   ...
   **/
    void registr(const std::string& n, const GiNaC::exvector& args, const unsigned h = 0,
   ...
   **/
    ...
    GiNaC::expression create(const std::string& n,
    ...
    static GiNaC::expression create_hard(const std::string& n,
                                         const GiNaC::exprseq& args = GiNaC::exprseq(),
...
private:
    ...
    struct funcrec
    {
        ...
        funcrec(const unsigned s, const GiNaC::exvector& v, const bool hc,
                const GiNaC::expression& d, const unsigned h, const std::string& n)
            : serial(s)
            , hard(hc)
            , vars(v)
            , definition(d)
        ...
        funcrec() { MSG_INFO(3, "Constructing empty funcrec" << endline); }
        ...
        ~funcrec() { MSG_INFO(3, "Destructing funcrec with serial " << serial << endline); }
#endif
    };
   ...
   **/
    static std::map<const std::string, funcrec>
        hard_functions; // Hard-coded functions that are the same for every instance of the Functionmanager
